<template>
	<view v-if="loaded" class="address-edit-root">
		<view class="address-content">
			...
			<view class="item-box">
				...
				<view class="item-right">
					<u-field v-model="address.country_str" @click="onClickSelectArea" :disabled="true" input-align="right" :label-width="0" :border-bottom="false" :placeholder="$t('placeholder.select')" :placeholder-style="placeholderStyle"></u-field>
				</view>
			</view>
			<view class="item-box">
				<view class="title">{{ $t('form.phone') }}</view>
				<view class="item-right">
					<u-field v-model="address.tel" type="number" input-align="right" :label-width="0" :border-bottom="false" :placeholder="$t('placeholder.fillin')" :placeholder-style="placeholderStyle"></u-field>
				</view>
			</view>
			<view class="item-box">
...
<script>
	export default {
		...
		onLoad(options) {
			this.addressId = options.id ? options.id : 0
		...
		methods: {
			fetchAddressDetail() {
				...
				this.$util.postRequest(this.$api.ud.addr_info, parameters, true,
					res => {
						this.address = res.data.row
						this.isDef = res.data.row.is_def == 1 ? true : false
						this.fetchDefValue()
						this.loaded = true
					},
					err => {
