		...
		<view class="paypwd-box">
			<view class="account-form">
				<u-form ref="withdrawForm" :model="modify" :error-type="['toast']" label-position="top">
					<u-form-item :label="$t('my.pay.password')" prop="realname" :required="true">
						<u-input v-model="modify.ppwd" type="password" passwordType="number" :placeholder="$t('my.pay.pwd.placeholder')"></u-input>
					</u-form-item>
	...
	export default {
		...
		methods: {
			...
			submitWithdrawForm() {
				...
				this.$util.postRequest(this.$api.user.modify_pay_pwd, params, true,
					res => {
						this.$util.showNoIconToast(res.info)
						this.$set(this.userInfo, 'hasPpwd', 1)
